...
(function initSocmedModal() {
  ...
  function openModal(card) {
    ...
    // Allow mouse wheel scroll inside modal info panel
    const infoPanel = qs('.smp-modal-info', modal);
  ...
  /* ── Allow wheel scroll inside smp-modal-info (bypass Lenis) ── */
  const smpInfoPanel = qs('.smp-modal-info', modal);
  if (smpInfoPanel) {
    smpInfoPanel.addEventListener('wheel', e => {
      e.stopPropagation();
      smpInfoPanel.scrollTop += e.deltaY;
    }, { passive: true });
  }
})();
...
document.addEventListener('DOMContentLoaded', function initLinkedInModal() {
  ...
  function openLiModal(e) {
    ...
    const panel = qs('.li-modal-panel', modal);
  ...
  /* ── Allow wheel scroll inside li-modal-panel (bypass Lenis) ── */
  const liPanel = qs('.li-modal-panel', modal);
  if (liPanel) {
    liPanel.addEventListener('wheel', e => {
      e.stopPropagation();
      liPanel.scrollTop += e.deltaY;
    }, { passive: true });
  }
...
(function initContactForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        btn.innerHTML = '<span>✅</span> Pesan Terkirim!';
        btn.style.backgroundColor = '#10b981';
        btn.style.borderColor = '#10b981';
        btn.style.color = '#ffffff';
        form.reset();
      } else {
        btn.innerHTML = '<span>❌</span> Gagal Dikirim!';
        btn.style.backgroundColor = '#ef4444';
        btn.style.borderColor = '#ef4444';
        btn.style.color = '#ffffff';
      }
    } catch (error) {
      btn.innerHTML = '<span>❌</span> Gagal Menyambung!';
      btn.style.backgroundColor = '#ef4444';
      btn.style.borderColor = '#ef4444';
      btn.style.color = '#ffffff';
    } finally {
...
/* ════════════════════════════════════════════════════════════
   PROJECT SHOW ALL LOGIC
════════════════════════════════════════════════════════════ */
(function initProjShowAll() {
  const showAllBtn  = qs('#projShowAll');
  const hideAllBtn  = qs('#projHideAll');
  const allGrid     = qs('#projAllGrid');

  /* ── "Lihat Semua Projek" toggle ─────────────────────── */
  if (showAllBtn && allGrid) {
    showAllBtn.addEventListener('click', () => {
      ...
      qsa('.reveal', allGrid).forEach(el => {
        if (!el.classList.contains('visible')) el.classList.add('visible');
      });

      // Bind LinkedIn modal for cards in allGrid that haven't been bound yet
      qsa('.btn-li-modal', allGrid).forEach(btn => {
        if (!btn.dataset.liBound) {
          btn.dataset.liBound = '1';
          btn.addEventListener('click', function(e) {
            ...
            qs('#liModal .li-project-title').innerHTML = d.liTitle || '';
            qs('#liModal .li-project-org').innerHTML   = d.liOrg  || '';
            qs('#liModal .li-project-dates').innerHTML = `<svg class="li-icon-svg" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="2" stroke="currentColor" stroke-width="1.5"/><path d="M5 1v2M11 1v2M2 6h12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>${d.liDate || ''}`;
            qs('#liModal .li-project-icon').innerHTML = d.liIcon || '📁';
            const mediaWrap = qs('#liModal .li-modal-media');
            if (d.liImg) {
              mediaWrap.innerHTML = `<img src="${d.liImg}" class="li-media-img" alt="${d.liTitle}">`;
            } else {
              mediaWrap.innerHTML = `<div class="li-media-ph"><span class="li-media-icon">${d.liIcon||'📁'}</span><span class="li-media-label">${d.liTitle||''}</span></div>`;
            }
            qs('#liModal .li-desc').innerHTML = d.liDesc || '';
            const skillsWrap = qs('#liModal .li-skills');
            skillsWrap.innerHTML = d.liSkills ? d.liSkills.split(',').map(s=>`<span>${s.trim()}</span>`).join('') : '';
            const extLink = qs('#liExtLink');
            if (d.liExtUrl) { extLink.style.display='flex'; extLink.href=d.liExtUrl; qs('#liExtTitle').innerHTML=d.liExtText||'Kunjungi Tautan'; try{ qs('#liExtUrl').innerHTML=(new URL(d.liExtUrl)).hostname; }catch(e){} } else { if(extLink) extLink.style.display='none'; }
            const projLink = qs('#liProjLink');
            if (d.liProjUrl) { projLink.style.display='flex'; projLink.href=d.liProjUrl; qs('#liProjTitle').innerHTML=d.liProjText||'Lihat Proyek'; try{ qs('#liProjUrlDomain').innerHTML=(new URL(d.liProjUrl)).hostname; }catch(e){} } else { if(projLink) projLink.style.display='none'; }
            modal.classList.add('open');
            document.body.style.overflow = 'hidden';
            if (window.lenis) window.lenis.stop();
            const panel = qs('.li-modal-panel', modal);
